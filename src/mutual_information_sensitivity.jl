...
second order sensitivity indices are given by:
...
From [^1], the total order sensitivity indices can be calculated as:
...
function _total_order_ci(Xi, Y, entropy_Y, discretization_entropy; n_bootstraps = 100, conf_level = 0.95)
    ...
    for i in 1:n_bootstraps
        ...
        mi_values[i] = (entropy_Y - get_conditional_entropy(Y_perm, Xi)) / (entropy_Y - discretization_entropy)
    end
...
function _first_order_ci(Xi, Y; n_bootstraps = 100, conf_level = 0.95)
    ...
    for i in 1:n_bootstraps
        Y_perm = Y[randperm(length(Y))]
        mi_values[i] = get_mutual_information(Xi, Y_perm) / get_entropy(Y_perm)
    end
...
function _second_order_ci(Xi, Xj, Y; n_bootstraps = 100, conf_level = 0.95)
    ...
    for i in 1:n_bootstraps
        Y_perm = Y[randperm(length(Y))]
        mi_values[i] = (get_conditional_mutual_information(Xi, Xj, Y_perm) - get_mutual_information(Xi, Xj)) / get_entropy(Y_perm)
    end
...
function _discretization_entropy(X::AbstractArray, f, batch; n_bin_configurations = 800, n_samples_per_configuration = 100)
    n_bins = size(discretize_values(X), 1)
    n_dims = size(X, 1)
    ...
    for _ in 1:n_bin_configurations
        ...
        entropy_Y += get_entropy(samples_Y)
    end
...
function _compute_mi(X::AbstractArray, f, batch::Bool, method::MutualInformation)
    ...
    if 0 in method.order
        @inbounds for i in 1:K
            Xi = @view X[i, :]
            total_order[i] = (entropy_Y - get_conditional_entropy(Y, Xi)) / (entropy_Y - discretization_entropy)
    ...
    if 1 in method.order
        ...
        @inbounds for i in 1:K
            Xi = @view X[i, :]
            first_order[i] = get_mutual_information(Xi, Y) / entropy_Y
    ...
    if 2 in method.order
        for (i, j) in combinations(1:K, 2)
            Xi = @view X[i, :]
            Xj = @view X[j, :]
            second_order[i,j] = second_order[j,i] = (get_conditional_mutual_information(Xi, Xj, Y) - get_mutual_information(Xi, Xj)) / entropy_Y
