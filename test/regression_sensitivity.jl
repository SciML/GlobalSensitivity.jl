...
p_range = [[-1, 1], [-1, 1]]
reg = gsa(linear_batch, RegressionGSA(), p_range; N = 1000, batch=true)

reg = gsa(linear, RegressionGSA(), p_range; N=1000, batch=false)
reg = gsa(linear, RegressionGSA(true), p_range; N=1000, batch=false)
