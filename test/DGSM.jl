...
f1(x) = x[1] + 2 * x[2] + 6.0 * x[3]
dist1 = [Uniform(4, 10), Normal(4, 23), Beta(2, 3)]
b = gsa(f1, DGSM(), dist1, samples = samples)
@test [b.a[1], b.absa[1], b.asq[1]] ≈ [1.0, 1.0, 1.0] atol = 10.0e-1
@test [b.a[2], b.absa[2], b.asq[2]] ≈ [2.0, 2.0, 4.0] atol = 10.0e-1
@test [b.a[3], b.absa[3], b.asq[3]] ≈ [6.0, 6.0, 36.0] atol = 10.0e-1
...
@test [c.asq[1]] ≈ [7.7] atol = 10.0e-1
@test [c.asq[2]] ≈ [24.5] atol = 10.0e-1
@test [c.asq[3]] ≈ [11.0] atol = 10.0e-1
@test [c.sigma[1]] ≈ [-14.7] atol = 10.0e-1
@test [c.sigma[2]] ≈ [-38.7] atol = 10.0e-1
@test [c.sigma[3]] ≈ [-42.0] atol = 10.0e-1
@test [c.tao[1]] ≈ [6.13] atol = 10.0e-1
@test [c.tao[2]] ≈ [16.94] atol = 10.0e-1
@test [c.tao[3]] ≈ [15.91] atol = 10.0e-1
...
@test [d.asq[1]] ≈ [310.9] atol = 10.0e-1
